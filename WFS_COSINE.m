function [wfs beta]=WFS_COSINE(Y,x,k,sigma)
% function [wfs beta]=WFS_COSINE(Y,x,k,sigma)
%
% Construct the k-th degree weighted cosine  series (WFS)
% representation on functional data Y(x). This is equivalent 
% to performing heat kernel smoothing with bandwidth sigma. 
% If sigma = 0, we have the usaul cosine Fourier series expansion.
%
...
%weight =diag([exp(-[0:k]'.^2*pi^2*sigma) ;exp(-[1:k]'.^2*pi^2*sigma)]); %add this line for cosine + sine basis
